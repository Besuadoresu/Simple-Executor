...
typedef struct global_State
{
    ...
    TString* strbufgc; // 36 // list of all string buffer objects
...
struct extra_Space
{
    extra_Space* previous;
    void* ptr_unknown;
    extra_Space* next;
    void* unknown;
    void* ref_count;
    void* lua_weakthreadref;
    uint16_t context_level;
    int unknowns[3];
...
struct lua_State
{
    ...
    LUAVM_SHUFFLE6(;,
        StkId top,             //8       /                    /* first free slot in the stack */
        StkId base,           //12                            /* base of current function */
        ls_globalstate<global_State*> global, //16       /
        CallInfo* ci,         //20       /                     /* call info for current function */
        StkId stack_last,    //24                             /* last free slot in the stack */
        StkId stack         //28         /                          /* stack base */
    );

    LUAVM_SHUFFLE2(;,
        CallInfo* end_ci,     //32                     /* points after end of ci array*/
        CallInfo* base_ci    //36                    /* array of CallInfo's */
    );

    LUAVM_SHUFFLE2(;,
        ls_stacksize<int> stacksize,      //40
        int size_ci         //44                     /* size of array `base_ci' */
    );
    
    LUAVM_SHUFFLE2(;,
        unsigned short nCcalls,   //48  /* number of nested C calls */
        unsigned short baseCcalls //50  /* nested C calls when resuming coroutine */
    );

    int cachedslot; //52   /* when table operations or INDEX/NEWINDEX is invoked from Luau, what is the expected slot for lookup? */

    LUAVM_SHUFFLE3(;,
        Table* gt,      //56     /* table of globals */
        UpVal* openupval,  //60  /* list of open upvalues in this stack */
        GCObject* gclist //64
    );

    TString* namecall; //68 /* when invoked from Luau using NAMECALL, what method do we need to invoke? */

    extra_Space* userdata; //72
};
// clang-format on
